...
Factory Droid does not support PostToolUse hooks. Log events yourself after each significant action.
...
1. Slug: `YYYYMMDD-HHMMSS` (UTC). Append keyword if non-empty.
2. Create `.claude/lessons/sessions/<slug>/` with `meta.json` (include `"platform":"droid"`),
   empty `arc.jsonl`, `counter` = `0`.
3. Write slug to `.claude/lessons/active-session`. Confirm. Begin logging.

### `/lesson-done`

Load session. Quality guard (warn < 8 events, stop if < 5 AND no nodes). Build graph inline.
Read profile. Decide web research. Generate from template. Write `output/<slug>.md`.
Run `python3 <plugin_root>/scripts/render_pdf.py .claude/lessons/output/<slug>.md`.
Update profile. Write `last-session`. Delete `active-session`.
...
Build `index.html` / concept `map.html`.
...
Foundations → concept → mermaid diagrams → fix → quiz (3–5 Q&A) → resources.

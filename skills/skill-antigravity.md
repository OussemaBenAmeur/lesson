# /lesson — Google Antigravity Skill
...
Google Antigravity does not support PostToolUse hooks. Log events yourself after each significant action.
...
2. Create `.claude/lessons/sessions/<slug>/` with `meta.json` (include `"platform":"antigravity"`),
   empty `arc.jsonl`, `counter` = `0`.
3. Write slug to `.claude/lessons/active-session`. Confirm. Begin logging.

### `/lesson-done`

Load session. Quality guard. Build graph inline. Read profile. Decide web research.
Generate from template. Write `output/<slug>.md`. Run `render_pdf.py`.
